    ...
    stage('Containerize') {
      steps {
        container('kaniko') {
          ...
          sh "/kaniko/executor --dockerfile Dockerfile --context `pwd` --skip-tls-verify --destination=${env.HARBOR_URL}/library/samples/golang:v1.0.${env.BUILD_ID}"
        }
      }
    }
...
//         writeFile file: 'anchore_images', text: "${env.HARBOR_URL}/library/samples/golang:v1.0.${env.BUILD_ID}"
      ...
      steps {
        container('nodejs') {
          sh """
            ...
            cd deploy
            # update values.yaml
            sed -i -r 's,repository: (.+),repository: ${env.HARBOR_URL}/library/samples/golang,' values.yaml
